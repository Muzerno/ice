...
export default function userManagement() {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "button",
            render: (item: any) => {
                return <><Button type="primary" className='!bg-yellow-300 mr-1' icon={<EditOutlined />} onClick={() => { setOpenModalEdit(true); formEdit.setFieldsValue({ ...item, role_id: item.role.id }); setSeleteUUid(item.id) }}></Button>
                    <Popconfirm
                        key={item.id}
