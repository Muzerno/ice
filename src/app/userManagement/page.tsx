          ...
          <>
            <Button
              ...
              onClick={() => {
                ...
                formEdit.setFieldsValue({ ...item, role_id: item.role.id });
    ...
    try {
      ...
      if (user.status === 200 && user.data.success) {
        ...
        if (user.data?.errorCode === "FK_USER_CAR") {
          messageApi.error(
            "ไม่สามารถลบผู้ใช้นี้ได้ เนื่องจากมีข้อมูลรถที่ผูกอยู่"
          );
        } else {
  ...
  return (
    <LayoutComponent>
      ...
      <Card
        key={"cardUser"}
        className="w-full"
        title={[<h1>จัดการข้อมูลผู้ใช้งาน</h1>]}
      >
                  ...
                  <Row>
                    <Col span={24}>
                      <Form.Item
                        ...
                        label="ระดับผู้ใช้"
                        rules={[
                          { required: true, message: "กรุณาเลือกระดับผู้ใช้" },
                        ]}
                      >
